...
def get_graph(p1, words1, words2):
        ...
        embeddings = []
        for word in words:
                embeddings.append(get_emb(word))

        mat = np.array(embeddings)
        vis = TSNE(n_components=3, perplexity=min(5, max(1, len(words1) - 1)), random_state=42).fit_transform(mat) # (n_turns*2, 3)

        colors = ["red", "blue"]
        color_indices = [i%2 for i in range(len(words))]
        ...
        if len(queues) > 0:
                ...
                with open("data/" + gameid, "w") as f:
                        json.dump({
                                ...
                                "p1": opp,
                                "scores": [],
                                "words": random.choices(words, k=15)
                        }, f)
        else:
...
def getimg():
        ...
        get_graph(player_raw, cur["h0"], cur["h1"])
